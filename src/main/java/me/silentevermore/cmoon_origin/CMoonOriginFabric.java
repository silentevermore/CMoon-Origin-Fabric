...
public class CMoonOriginFabric implements ModInitializer{
	...
	public void onInitialize(){
		...
		AttackEntityCallback.EVENT.register((player, world, hand, entity, hitResult)->{
            //prevent player from hitting others, if has C-Moon origin
			OriginComponent originComponent = ModComponents.ORIGIN.get(player);
			Origin origin=originComponent.getOrigin(OriginLayers.getLayer(Identifier.of(Origins.MODID, "origin")));
			if (origin!=null && origin.getIdentifier().equals(Identifier.of(MOD_ID, "cmoon"))){
				if (entity.isLiving()) ((LivingEntity)entity).addStatusEffect(new StatusEffectInstance(ModEffects.INVERSED_LIMBS, 100, 1, false, false));
